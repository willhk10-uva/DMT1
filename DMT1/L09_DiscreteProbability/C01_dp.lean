...
@[simp]
def ProbabilityMeasure.P (A : Event Ω) : ℝ :=
...
def ProbabilityMeasure.variance (X : DRandom Ω ℝ) : ℝ :=
  let μX := μ.expectation X
  μ.expectation (fun ω => (X ω - μX)^2)

/-- Indicator function of an event. -/
@[simp]
def indicator {Ω : Type} (A : Event Ω) : Ω → ℝ :=
...
lemma expectation_indicator_eq_prob (A : Event Ω) :
  ...
  have h_sum : μ.space.sum (fun ω => if ω ∈ A then μ.prob ω else 0) =
    ...
    -- Use `Finset.sum_filter` in the desired direction
